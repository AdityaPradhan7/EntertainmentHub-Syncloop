...
const syncloop_api = 'https://cloud.syncloop.com/tenant/1736837793568/packages.UltimateEntertainmentHub.APIs.OpenLibrary1.main';
...
const BooksApp = () => {
  ...
  const handleGenreChange = async (genre) => {
    ...
    if (genre === 'all') {
      // Fetch trending books if "All Genres" is selected
      // const response = await axios.get(
      //   'https://cloud.syncloop.com/tenant/1736837793568/packages.UltimateEntertainmentHub.APIs.OpenLibrary_trending.main',
      //   {
      //     headers: {
      //       Authorization: `Bearer ${Bearer_Token}`,
      //     },
      //   }
      // );
      // if (response.data?.trending?.works) {
      //   const trendingBooks = response.data.trending.works.map((book) => ({
      //     id: book.key,
      //     title: book.title,
      //     titleUrl: `https://openlibrary.org/books/${book.cover_edition_key}`,
      //     authorNames: book.author_name || [],
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `${syncloop_api}?q=${encodeURIComponent(
          searchQuery
        )}`,
        {
          headers: {
